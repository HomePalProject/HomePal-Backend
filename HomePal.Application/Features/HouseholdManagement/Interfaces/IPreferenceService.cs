...
{
    Task<Result<IReadOnlyCollection<PreferenceResponse>>> GetAllPreferencesAsync(CancellationToken cancellationToken = default);
    Task<Result<IReadOnlyCollection<PreferenceResponse>>> SearchPreferencesAsync(string? query, CancellationToken cancellationToken = default);
